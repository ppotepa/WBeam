````markdown
# WBeam Trainer — Training, Encoding & Decoding Profile System Specification
...
1. Should `mjpeg` be enabled by default or opt-in only?
2. Should profile namespaces automatically include device class tags?
3. Should bitrate safety factor be global, mode-specific, or device-specific?
4. Should baseline warmup default to 60s or 90s?
5. Should old profiles auto-revalidate when host/app version changes?
6. Should there be a separate “fast search” mode for quick field use?
7. Should profiles store both “peak” and “validated sustained” winners?
8. Should transport class influence default codec ordering?
9. Should artifact retention support automatic pruning rules?
10. Should diagnostics-only runs be first-class stored objects?
