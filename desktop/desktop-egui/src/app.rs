...
use crate::models::{DeviceInfo, EventLevel, StatusEvent, StreamStats};
use crate::services::{
    detect_runtime, hostname, now_string, read_adb_devices, read_json, read_stream_stats,
    write_json,
};
...
impl DesktopApp {
    pub(crate) fn new(proto_root: PathBuf) -> Self {
        ...
        app.add_event(
            EventLevel::Info,
            format!(
                "desktop app started (os={} session={} host={})",
                app.os_name, app.session_type, app.host_name
            ),
        );
        app
    }
    ...
    pub(crate) fn save_settings(&mut self) {
        let backend = self.backend_value.trim();
        ...
        cfg["PROTO_CAPTURE_BACKEND"] = Value::String(backend.to_string());
        ...
        match write_json(&self.config_path, &cfg) {
            Ok(()) => {
                self.status_line = format!("saved {}", self.config_path.display());
                self.add_event(
                    EventLevel::Info,
                    format!(
    ...
    pub(crate) fn run_proto(&mut self) {
        match self.spawn_runner() {
            Ok(mode) => {
                self.status_line = format!("launched via {mode}");
                self.add_event(EventLevel::Info, format!("run launched via {mode}"));
            }
            Err(err) => {
    ...
    pub(crate) fn run_host_only_rust(&mut self) {
        ...
        match self.spawn_with_log(&mut cmd) {
            Ok(_) => {
                self.status_line =
                    format!("host (rust) launched, log: {}", self.runner_log.display());
                self.add_event(
                    EventLevel::Info,
                    format!("host only launched, log: {}", self.runner_log.display()),
                );
            }
            Err(err) => {
