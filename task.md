...
[soak:loopback 20260219-231818] PASS | 10s | fps_low_windows=0/10 | crc_errors=0 | late_frames=0 | log=/mnt/fat_boii/git/WBeam/protocol/logs/soak_loopback_20260219-231818.log
33. `2026-02-21 00:00 | host | B3 | host/rust/crates/wbeamd-core/src/lib.rs, host/rust/crates/wbeamd-core/src/domain/, host/rust/crates/wbeamd-core/src/infra/ | SOLID refactor wbeamd-core: split monolithic lib.rs into domain/(state.rs, adaptation.rs, session.rs, telemetry.rs) and infra/(config.rs, process.rs, api_types.rs); lib.rs becomes thin re-export; all 17 unit tests pass, cargo build --workspace clean | cargo test 17 passed; cargo build Finished | B3 Android`
34. `2026-02-21 00:30 | host | B3 | host/rust/crates/wbeamd-streamer/src/ | SOLID refactor wbeamd-streamer: new modules cli.rs, capture.rs, encoder.rs, pipeline.rs, transport.rs; main.rs reduced to 91-line orchestrator only; no logic in main, each module owns single concern | cargo build Finished clean | Android SOLID refactor`
35. `2026-02-21 01:00 | android | B3 | android/app/src/main/java/com/wbeam/ | SOLID refactor Android: extract api/HostApiClient.java (OkHttp pool+retry), api/StatusPoller.java (background poll), api/StatusListener.java (callback interface), settings/SettingsRepository.java+SettingsSnapshot, stream/StreamSessionController.java, stream/H264TcpPlayer.java, telemetry/ClientMetricsReporter.java, ClientMetricsSample.java; MainActivity 3084→1568 lines (49% reduction) | ./gradlew assembleDebug BUILD SUCCESSFUL in 9s | wbtp-host`
36. `2026-02-21 01:30 | android | B3 | android/app/src/main/java/com/wbeam/stream/VideoTestController.java | extract VideoTestController from MainActivity: public video test + bandwidth test with Callbacks interface; BandwidthResult + TestConfig inner classes; startPublicVideoTest, startBandwidthTest, release; MainActivity field+init added | ./gradlew assembleDebug BUILD SUCCESSFUL | wbtp-host crate`
37. `2026-02-21 02:00 | protocol | B3 | protocol/rust/crates/wbtp-host/Cargo.toml, protocol/rust/crates/wbtp-host/src/lib.rs | create missing wbtp-host crate (workspace was broken): HostSender (sync push API, Arc inner), HostSenderConfig, AtomicStats; wraps wbtp-sender FrameSender with drop-oldest queue; cargo test 7/7 pass (wbtp-core×4, wbtp-host×2, doc×1), cargo build --workspace clean | cargo test 7 passed; cargo build Finished | soak script`
38. `2026-02-21 02:30 | qa | B3 | protocol/scripts/soak_loopback_30m.sh | create automated 30m loopback soak gate: builds wbtp-receiver-null+wbtp-sender-null in release, runs sender→receiver on loopback, heartbeat every 60s, PASS/FAIL on crc_errors=0, magic_errors=0, windows>=duration×0.9, low_fps<=3%; fix grep pattern ': metrics ' (not '"metrics"'); smoke test PASS 31 windows 0 errors; 30m soak launched | SOAK_DURATION=30 PASS | wire wbtp-core into streamer`
39. `2026-02-21 03:00 | host | B3 | host/rust/crates/wbeamd-streamer/Cargo.toml, host/rust/crates/wbeamd-streamer/src/transport.rs | wire wbtp-core into wbeamd-streamer: add wbtp-core path dep + bytes dep; replace hand-rolled build_header (manual Cursor/WriteBytesExt) with wbtp_core::FrameHeader::encode via BytesMut; remove FRAME_MAGIC/FRAME_VERSION/FRAME_FLAG_KEYFRAME local constants; HELLO+server loop unchanged; ensures wire format stays in sync with reference impl | cargo build -p wbeamd-streamer + cargo build --workspace both Finished clean | check 30m soak result`
