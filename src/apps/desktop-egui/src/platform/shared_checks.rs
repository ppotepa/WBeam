...
use crate::services::{
    adb_available, detect_os_version, detect_runtime, display_probe_summary, hostname,
    portal_has_interface, read_adb_devices, user_service_is_active,
};
...
#[derive(Default)]
pub(crate) struct SessionProbeStartupCheck;

impl StartupCheck for SessionProbeStartupCheck {
    fn id(&self) -> &'static str {
        "session_probe"
    }

    fn run(&self, state: &mut BootstrapState) -> anyhow::Result<Option<MonitorEvent>> {
        let summary = display_probe_summary(&state.session_type);
        Ok(Some(MonitorEvent::Info {
            source: self.id(),
            message: summary,
        }))
...
#[derive(Default)]
pub(crate) struct PortalScreencastStartupCheck;

impl StartupCheck for PortalScreencastStartupCheck {
    fn id(&self) -> &'static str {
        "portal_screencast_probe"
    }

    fn run(&self, state: &mut BootstrapState) -> anyhow::Result<Option<MonitorEvent>> {
        if state.os_name != "linux" {
            return Ok(None);
        }

        let has_screencast = portal_has_interface("org.freedesktop.portal.ScreenCast");
        if has_screencast {
            return Ok(Some(MonitorEvent::Info {
                source: self.id(),
                message: "portal ScreenCast interface detected".to_string(),
            }));
        }

        let portal_active = user_service_is_active("xdg-desktop-portal.service")
            .map(|v| if v { "active" } else { "inactive" })
            .unwrap_or("unknown");
        let kde_active = user_service_is_active("plasma-xdg-desktop-portal-kde.service")
            .map(|v| if v { "active" } else { "inactive" })
            .unwrap_or("unknown");
        let gtk_active = user_service_is_active("xdg-desktop-portal-gtk.service")
            .map(|v| if v { "active" } else { "inactive" })
            .unwrap_or("unknown");
        ...
        Ok(Some(MonitorEvent::Warn {
            source: self.id(),
            message: format!(
                "ScreenCast portal missing (session={}): xdg-desktop-portal={} kde-backend={} gtk-backend={}",
                state.session_type, portal_active, kde_active, gtk_active
            ),
        }))
    }
}
