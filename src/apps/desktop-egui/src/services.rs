...
pub(crate) fn display_probe_summary(session_type: &str) -> String {
    let display = env::var("DISPLAY").unwrap_or_else(|_| "-".to_string());
    let wayland = env::var("WAYLAND_DISPLAY").unwrap_or_else(|_| "-".to_string());
    let session_id = env::var("XDG_SESSION_ID").unwrap_or_else(|_| "-".to_string());
    let desktop = env::var("XDG_CURRENT_DESKTOP").unwrap_or_else(|_| "-".to_string());
    let remote = if env::var_os("XRDP_SESSION").is_some() || env::var_os("SSH_CONNECTION").is_some() {
        "yes"
    } else {
        "no"
    };
    format!(
        "session={session_type} display={display} wayland={wayland} xdg_session={session_id} desktop={desktop} remote={remote}"
    )
}

pub(crate) fn portal_has_interface(interface_name: &str) -> bool {
    if env::consts::OS != "linux" {
        return false;
    }
    ...
    match output {
        Ok(out) if out.status.success() => {
            ...
            text.contains(interface_name)
        }
        _ => false,
    }
}

pub(crate) fn user_service_is_active(unit: &str) -> Option<bool> {
