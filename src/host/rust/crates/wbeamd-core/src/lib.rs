...
fn load_presets_from_training_files(root: &Path) -> Option<BTreeMap<String, ActiveConfig>> {
    ...
    // Legacy fallback: archived proto lane.
    let candidates = [
        root.join("config/training/profiles.json"),
        root.join("archive/experimental/proto/config/profiles.json"),
    ];
    ...
    warn!("presets: no training profiles found in config/training or archive/experimental/proto/config");
    None
}
