...
def proto_best_to_profile_doc(
    *,
    ...
    now = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    fps = max(24, as_int(best_cfg.get("PROTO_CAPTURE_FPS"), 60))
    bitrate_kbps = max(1000, as_int(best_cfg.get("PROTO_CAPTURE_BITRATE_KBPS"), 10000))
    size = normalize_size(str(best_cfg.get("PROTO_CAPTURE_SIZE", "1280x720")))
    cursor_mode = str(best_cfg.get("PROTO_CURSOR_MODE", "embedded")).strip() or "embedded"
    encoder = "h264" if as_int(best_cfg.get("PROTO_H264"), 1) == 1 else "h265"
    gop = max(1, as_int(best_cfg.get("WBEAM_H264_GOP"), max(1, fps // 2)))
    values_keys = (
        "PROTO_CAPTURE_SIZE",
        "PROTO_CAPTURE_FPS",
        ...
        "PROTO_PORTAL_RESTORE_TOKEN_FILE",
    )
    quality_keys = ("PROTO_CAPTURE_BITRATE_KBPS", "WBEAM_H264_GOP", "WBEAM_APPSINK_MAX_BUFFERS")
    latency_keys = (
    ...
    values = {k: best_cfg[k] for k in values_keys if k in best_cfg}
    quality = {k: best_cfg[k] for k in quality_keys if k in best_cfg}
    latency = {k: best_cfg[k] for k in latency_keys if k in best_cfg}
    quality.setdefault("WBEAM_H264_GOP", gop)
    return {
        "schema_version": 1,
        "profile_name": profile_name,
        "generated_at": now,
        ...
        "device": {"serial": serial, "stream_port": stream_port},
        "profile": {
            "description": "Auto-generated by trainer wizard (proto engine).",
            "origin": {"generated_at": now, "source": "legacy_engine"},
            "runtime": {
                "encoder": encoder,
...
def run_proto_autotune_engine(
    *,
    ...
    profile_doc = proto_best_to_profile_doc(
        profile_name=profile_name,
        best_cfg=best_cfg,
        serial=serial,
        stream_port=stream_port,
    ...
    parameters_doc = {
        ...
        "artifacts": {
            ...
            "profiles_file": str(profiles_out_path),
            "results_file": str((ROOT / "proto" / results_name).resolve()),
